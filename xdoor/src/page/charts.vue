    ...
    <el-row>
      <el-col :span="24">
        ...
        <el-table
          ...
          style="width: 100%"
          size="small">
          <el-table-column prop="feePayType" label="类别" width="180" align="center">
            <template slot-scope="scope">
              <p v-if="scope.row.feePayType == 1">刷卡</p>
              <p v-if="scope.row.feePayType == 2">现金</p>
              <p v-if="scope.row.feePayType == 3">其他</p>
              <p
                v-if="scope.row.feePayType != 1 && scope.row.feePayType != 2 &&scope.row.feePayType != 3 "
              >其他</p>
            </template>
          </el-table-column>

          <el-table-column label="金额" width="240" show-overflow-tooltip align="center">
            <template slot-scope="scope">
              <p
                v-if="scope.row.feePayTowards == 1"
                style="color: #58D68D"
              >+{{scope.row.feePayValue}}</p>
              <p
                v-if="scope.row.feePayTowards == -1"
                style="color: #EC7063"
              >-{{scope.row.feePayValue}}</p>
          ...
          <el-table-column label="操作" fixed="right">
            <template slot-scope="scope">
              <el-button
                ...
                v-if="scope.row.feePayStatus == 1"
                @click.stop="ensure(scope.row)"
              >确定</el-button>
            </template>
          </el-table-column>
        </el-table>
            ...
            for (let i = 0; i < res.data.length; i++) {
              ...
              if (map.orderRate || map.rentRate) {
                data.push(
                  parseFloat(
                    (
                      parseFloat(map.orderRate) + parseFloat(map.rentRate)
                    ).toFixed(2)
                  )
                );
              } else {
